        ...
        <table style={{
          ...
          <thead>
            <tr style={{ background: 'var(--paper-soft)' }}>
              <th style={{ padding: '16px 20px', textAlign: 'left', fontSize: 13, color: 'var(--muted)', fontWeight: 700, width: 180, borderBottom: '1px solid var(--line)' }}>
                Параметр
              </th>
              {items.map(item => (
                <th key={item.id} style={{ padding: '16px 20px', textAlign: 'left', borderBottom: '1px solid var(--line)', minWidth: 220 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                      <Badge tone="neutral">{item.institution_type === 'college' ? 'Колледж' : 'Университет'}</Badge>
                      <button
                        onClick={() => toggle(item)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted-light)', fontSize: 16, padding: 0 }}
                        title="Убрать из сравнения"
                      >✕</button>
                    </div>
                    <strong style={{ fontSize: 15, color: 'var(--text)' }}>{item.specialty_title || item.title || '—'}</strong>
                    <span style={{ fontSize: 13, color: 'var(--muted)' }}>{item.institution_name || '—'}</span>
                  </div>
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {ROWS.map((row, rowIndex) => (
              <tr key={row.label} style={{ background: rowIndex % 2 === 0 ? 'var(--paper)' : 'var(--paper-soft)' }}>
                <td style={{ padding: '14px 20px', fontSize: 13, color: 'var(--muted)', fontWeight: 600, borderBottom: '1px solid var(--line)' }}>
                  {row.label}
                </td>
                {items.map(item => {
